        ...
        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $key+=1 }}</td>
                    <td>
                        <a class="image-popup-no-margins" href="{{asset("/images/header/".$header->image)}}">
                            <img class="img-fluid rounded img-thumbnail" alt="" src="{{asset("/images/header/".$header->image)}}" width="90">
                        </a>
                    </td>
                    <td>{{ $header->title }}</td>
                    <td>{{ $header->body }}</td>
                    <td><a href="{{ route("admin.header.edit",1) }}" class="btn btn-primary">تعديل</a></td>
                </tr>
                @endforeach
            </tbody>
        </table>
